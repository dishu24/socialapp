...
function follow_user(element,username,place){
    ...
    .then(() => {
        ...
        if(place === 'edit_page'){
            element.parentElement.innerHTML= `<button class="btn btn-success" type="button" onclick="unfollow_user(this,'${username}','edit_page')" id="following-btn">Following</button>`;
        }
...
function unfollow_user(element,username,place){
    fetch('/'+username+'/unfollow',{
        method:'PUT'
    })
    .then(() => {
        if(place === 'suggestion'){
            element.parentElement.innerHTML= `<button class="btn btn-success" type="button" onclick="follow_user(this,'${username}','suggestion')">Follow</button>`;
        }
        if(place === 'edit_page'){
            element.parentElement.innerHTML= `<button class="btn btn-success" type="button" onclick="follow_user(this,'${username}','edit_page')" id="following-btn">Follow</button>`;
        }
        if(document.querySelector('.body').dataset.page === 'profile'){
            if(document.querySelector('.profile-view').dataset.user=== username){
                document.querySelector('#follower__count').innerHTML--;
...
function createpost(){
    ...
    pops.querySelector('.large-popup').style.display= "block";
    document.querySelector('.body').setAttribute('aria-hidden','true');
    document.querySelector('body').style.overflow = "hidden";
    document.querySelector('#insert-img').onchange = previewFile;
    document.querySelector('.large-popup').querySelector('form').setAttribute('onsubmit','');
    document.querySelector('.large-popup').querySelector('#post-text').addEventListener('input',(event) => {
        if (event.target.value.trim().length > 0) {
            pops.querySelector('.submit-btn').disabled= false;
        }
        else if (event.target.parentElement.querySelector('#img-div').style.backgroundImage) {
            pops.querySelector('.submit-btn').disabled= false;
        }
        else {
            pops.querySelector('.submit-btn').disabled= true;
        }
    });
...
function confirm_delete(id){
    ...
    let pops = document.querySelector('.popup');
    pops.style.display = 'block';
    let small_popup =pops.querySelector('.small-popup');
...
function edit_post(element){
    ...
    let pops = document.querySelector('.large-popup');
    let promise = new Promise((resolve,reject) => {
        let post_text = post.querySelector('.post-content').innerHTML;
        let post_image = post.querySelector('.post-image').style.backgroundImage;

        pops.querySelector('#post-text').value = post_text;
        if (post_image) {
            pops.querySelector('#img-div').style.backgroundImage = post_image;
            document.querySelector('#del-img').addEventListener('click', del_image);
            pops.querySelector('#img-div').style.backgroundImage = 'block';

        }
        else {
            pops.querySelector('#img-div').style.backgroundImage = '';
        }
        resolve(pops);
    });
    promise.then( () => {
        createpost();
        pops.querySelector('form').setAttribute('onsubmit', `return edit_post_submit(${post.dataset.post_id})`)
        pops.querySelector('.submit-btn').disabled = false;
    });
}
...
function edit_post_submit(post_id){
    // debugger;
    let pops = document.querySelector('.large-popup');
    let text = pops.querySelector('#post-text').value;
    let pic = pops.querySelector('#insert-img');
    let change_pic = pops.querySelector('#img-change');
    let formdata = new FormData();
    formdata.append('text', text);
    formdata.append('picture', pic.files[0]);
    formdata.append('img-change', change_pic.value);
    formdata.append('id', post_id);

    fetch('post/'+parseInt(post_id)+'/edit', {
        method: "POST",
        body: formdata
    })
...
function show_comment(element) {
    ...
    fetch('post/'+parseInt(post_id)+'/comments')
    .then(response => response.json())
    .then(comments => {
...
function write_comment(element) {
    ...
    fetch('post/'+parseInt(post_id)+'/write_comment',{
        method: 'POST',
        body: JSON.stringify({
